...
class SystemActionsDialog extends ModalDialog {
    _init(settings) {
        ...
        this._dimHandlerId = this._settings.connect('changed::backdrop-dim', () => this._applyBackdropDim());
    ...
    _applyBackdropDim() {
        let dim = 0.5;
        try { dim = this._settings.get_double('backdrop-dim'); } catch (_e) {}
        if (Number.isNaN(dim)) dim = 0.5;
        dim = Math.max(0, Math.min(1, dim));
        const lb = this._lightbox;
        if (!lb) return;
        try {
            // Radial lightbox (used by ModalDialog): the 'brightness' uniform
            // is the dim multiplier (0 = no dim, 0.5 = stock shell look).
            if (lb.get_effect('radial')) {
                const props = {duration: 250, mode: Clutter.AnimationMode.EASE_OUT_QUAD};
                lb.ease_property('@effects.radial.brightness', dim, props);
                return;
            }
            // Non-radial fallback shades by actor opacity.
            lb.set_opacity(Math.round(255 * dim));
        } catch (_e) {}
    }
    ...
    _applyPanelOpacity() {
        ...
        // Uniform transparency for the whole popup: fade every surface
        // (background, header, buttons, text) proportionally. This reads as
        // "glass" far better than only translucenting the sa-panel node, whose
        // inner surfaces would otherwise stay solid and mask the effect.
        const target = this.contentLayout ?? this._styleRoot ?? this.dialogLayout;
        if (target) target.set_opacity(Math.round(opacity * 255));
    }
