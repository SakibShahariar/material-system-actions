...
export class TuiStyle extends BaseStyle {
    buildUI() {
        const WIDTH = 42;
        const box = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, style_class: 'sa-panel-flat sa-mono sa-layout-tui', style: 'min-width: 400px; padding: 10px; font-family: monospace;'});
        ...
        for (let i = 0; i < this.actions.all.length; i++) {
            ...
            const btn = new St.Button({style_class: 'sa-tui-row', can_focus: true, x_expand: true, style: 'padding: 0; border: none;'});
            btn.set_child(label);
    ...
    onKeyPress(sym) {
        if (!this._rows.length)
            return false;
        if (sym === Clutter.KEY_Up || sym === Clutter.KEY_Left || sym === Clutter.KEY_k) { this._select(this._idx - 1); return true; }
        if (sym === Clutter.KEY_Down || sym === Clutter.KEY_Right || sym === Clutter.KEY_j) { this._select(this._idx + 1); return true; }
