...
export class BentoStyle extends BaseStyle {
    buildUI() {
        const outer = new St.BoxLayout({style_class: 'sa-panel-flat sa-grid-seam sa-layout-bento', style: 'spacing: 2px; padding: 8px;'});
        // Use vertical box to emulate grid: 2 rows
        const col0 = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, style: 'spacing: 1px; width: 36px;'});
        ...
        const main = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, style: 'spacing: 1px;'});
        const topRow = new St.BoxLayout({style: 'spacing: 1px;'});
        if (hero) {
            const heroBtn = this._tile(hero, 180, 76, 30, true);
            topRow.add_child(heroBtn);
        }
        const clockBox = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, style: 'padding: 12px; min-width: 110px;', y_align: Clutter.ActorAlign.CENTER});
    ...
    _tile(action, w, h, iconSize, hero) {
        ...
        const content = new St.BoxLayout({orientation: Clutter.Orientation.VERTICAL, x_align: Clutter.ActorAlign.CENTER, y_align: Clutter.ActorAlign.CENTER, style: 'spacing: 8px;'});
