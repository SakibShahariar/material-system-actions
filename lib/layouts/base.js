...
export class BaseStyle {
    ...
    _confirmThenRun(action) {
        ...
        confirmBtn.connect('clicked', () => {
            cleanup();
            GLib.timeout_add(GLib.PRIORITY_DEFAULT, 150, () => { this._run(action.cmd); return GLib.SOURCE_REMOVE; });
        });
        // Esc cancels, Enter confirms (handled via dialog's global handler forwarding to focused button)
        GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            try { cancelBtn.grab_key_focus(); } catch (_e) {}
            return GLib.SOURCE_REMOVE;
        });
        // also wire dialog Esc to cleanup if confirm visible
        const origClose = this.dialog.close.bind(this.dialog);
        const escId = this.dialog.connect('key-press-event', (_a, evt) => {
            if (evt.get_key_symbol() === Clutter.KEY_Escape) {
                this.dialog.disconnect(escId);
                cleanup();
