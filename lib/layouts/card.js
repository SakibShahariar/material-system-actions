...
export class CardStyle extends BaseStyle {
    buildUI() {
        const box = new St.BoxLayout({vertical: true, style_class: 'sa-panel sa-layout-card', style: 'width: 780px; min-width: 780px; max-width: 780px; padding: 20px 24px 18px; spacing: 0;'});

        // Header pill — avatar + host + clock
        const header = new St.BoxLayout({style_class: 'sa-header-pill sa-card-header', x_expand: true});
        ...
        box.add_child(new St.Label({text: 'System Actions', style_class: 'sa-card-title', x_align: Clutter.ActorAlign.CENTER}));
    ...
    _section(title, actions) {
        const sec = new St.BoxLayout({vertical: true, style: 'spacing: 8px; margin-top: 16px;'});
        sec.add_child(new St.Label({text: title, style_class: 'sa-heading', x_align: Clutter.ActorAlign.CENTER}));
        const row = new St.BoxLayout({x_align: Clutter.ActorAlign.CENTER, style: 'spacing: 12px;'});
    ...
    _pill(action) {
        ...
        const content = new St.BoxLayout({style: 'spacing: 10px;', x_align: Clutter.ActorAlign.CENTER, y_align: Clutter.ActorAlign.CENTER});
        content.add_child(this.makeIcon(action.icon, 20));
        content.add_child(new St.Label({text: action.label, style_class: 'sa-pill-label', x_align: Clutter.ActorAlign.CENTER, y_align: Clutter.ActorAlign.CENTER}));
