...
export class HaloStyle extends BaseStyle {
    buildUI() {
        ...
        const outer = new St.BoxLayout({vertical: true, style_class: 'sa-panel', style: 'spacing: 16px; padding: 20px;', x_align: Clutter.ActorAlign.CENTER});
    ...
    onKeyPress(sym) {
        if (sym === Clutter.KEY_space || sym === Clutter.KEY_Return || sym === Clutter.KEY_KP_Enter) {
            // space hold not easy without key-release capture; treat as cycle then hold via timeout
            this._onPress();
            GLib.timeout_add(GLib.PRIORITY_DEFAULT, this.HOLD_MS, () => { this._onRelease(); return GLib.SOURCE_REMOVE; });
            return true;
        }
        if (sym === Clutter.KEY_Tab || sym === Clutter.KEY_Right || sym === Clutter.KEY_Left) {
            // Tab cycles target like short tap
